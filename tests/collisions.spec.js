...
import { Plateform, Player } from "../src/solids";

describe('Collision', () => {
    it('should detect player on ground', () => {
        expect(isPlayerCollidesVertically(new Player(200, 200), new Plateform(0, 220, 300, 50))).toBeTruthy();
    });
    it('should not collide', () => {
        expect(isPlayerCollidesVertically(new Player(20, 5), new Plateform(0, 50, 100, 10))).toBeFalsy();
    });

    it('should resolve vertically by top', () => {
        const player = new Player(200, 190);
        player.y = 210;
        const platform = new Plateform(0, 220, 300, 50);
    ...
    it('should resolve vertically by bottom', () => {
        ...
        player.y = 15;
        const platform = new Plateform(0, 0, 300, 50);
        expect(isPlayerCollidesVertically(player, platform)).toBeTruthy();
        resolve(player, platform, ResolveSolution.Y);
    ...
    it('should resolve horizontally by left', () => {
        ...
        player.x = 98;
        player.velX = 5;
        const platform = new Plateform(100, 0, 300, 50);
    ...
    it('should resolve horizontally by right', () => {
        ...
        player.x = 0;
        const platform = new Plateform(0, 0, 1, 400);
        expect(isPlayerCollidesHorizontally(player, platform)).toBeTruthy();
        resolve(player, platform, ResolveSolution.X);
